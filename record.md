...
### Exp-104：A2 按历史长度桶选择模型融合权重

日期：2026-07-03

目的：

- 验证 A2 是否应该对 `len=0/1/2-3/4-10/>10` 五类用户使用不同的模型融合权重。
- 动机是测试集历史长度分布与训练集差异极大，单一 `model_weight` 可能无法同时兼顾冷启动和长历史用户。

快速多 split 审计结果：

- 底座：Exp044 feature ranker checkpoint。
- 验证方式：test-like validation，split seeds = `42,777,2024`。
- LOSO 均值收益：`-0.000376`。
- 最差 split 收益：`-0.003188`。
- 正收益 split：`2/3`。

结论：

- 该方向离线不稳定，最差折损失明显，不满足提交门槛。
- 已删除临时代码 `framework/code/a2_bucket_model_weight.py`，不进入正式路线。
- 后续 A2 不再做按验证集直接挑每个长度桶权重的微调，除非有更强的 OOF 或线上反馈支撑。


### Exp-105：A2 全组合 EB 冷启动候选
...
- 继续优化 A2 冷启动用户。
- 旧 Exp070 使用用户画像前缀组合：`u_cat_01 -> u_cat_01,u_cat_02 -> u_cat_01,u_cat_02,u_cat_03`。
- 本实验改为搜索所有 1/2/3 列用户画像组合，验证有效画像组合是否不在固定前缀中。

代码调整：

- 新增 `framework/code/a2_combo_eb.py`。
- 只服务冷启动/短历史 EB，不加载神经模型，不影响现有训练脚本。
- 使用 pandas groupby 统计组合 target 分布。
- 搜索阶段缓存每个验证用户可命中的组合 counter，避免每组参数重复查 92 个组合。

审计结果：

- 同样 1000 样本、5 split 小网格对照：
  - prefix mean NDCG：`0.358806`
  - all-combo mean NDCG：`0.361119`
  - all-combo 相对 prefix：约 `+0.002313`
- 全组合最佳参数：
  - `alpha=100`
  - `min_count=20`
  - `global_weight=1.0`
  - `weights_by_size={1:0.2, 2:0.5, 3:1.0}`
...
- 只替换 `len=0`。
- `eb_weight=1.0` 过激：
  - 总 `top1_changed=11.27%`
  - `len=0 top1_changed=32.06%`
- `eb_weight=0.7` 较安全：
  - 总 `top1_changed=1.86%`
  - `len=0 top1_changed=5.29%`
  - Top10 集合 overlap `100%`，主要是重排而不是大换列表。

候选包：

- `framework/output/exp105_a1_exp090_a2_combo_eb_w07/prediction.zip`
- 格式校验通过。

结论：

- 全组合 EB 有正信号，但幅度不大。
- 暂不作为单独提交推荐；除非同时拿到 A1 新提升，否则不建议消耗提交机会。
- 该方向可作为 A2 小幅候选保留，风险低于大面积替换模型。


### Exp-106：A1 Label Smoothing 审计

日期：2026-07-03

目的：

- 验证官方建议中的 Label Smoothing 是否能缓解 A1 SIGN + 标签传播特征的过拟合。
- 只改训练损失，不改 SIGN 特征、不改 C&S、不改推理逻辑。
...
- split seeds：`42,777,2024,2026,3407`

GPU 返回结果：

| candidate | mean | min | max | std |
| --- | ---: | ---: | ---: | ---: |
| `undir_reverse_h3_rw_ls0p03` | 0.759087 | 0.745205 | 0.778082 | 0.011687 |
| `undir_reverse_h3_rw_ls0p05` | 0.757626 | 0.744292 | 0.776256 | 0.010846 |
| `undir_h3_rw_ls0p03` | 0.757443 | 0.749772 | 0.776256 | 0.009571 |
| `undir_h3_rw_ls0p05` | 0.757260 | 0.748858 | 0.776256 | 0.009758 |

对比：

- Exp059/Exp060 主线中，`undir_reverse/all-combo` 相关配置离线均值约 `0.763-0.764`。
- Exp106 最好只有 `0.759087`，低于现有 A1 主线。
