...
class ToolRegistry:
    ...
    def _run_inference(self, code_dir: str, task_id: int, checkpoint_path: str,
        ...
        """执行推理脚本"""
        task_arg = task_id if isinstance(task_id, str) and task_id.startswith("task") else f"task{task_id}"
        cmd = [sys.executable, os.path.join(code_dir, "infer.py"), "--task", task_arg,
