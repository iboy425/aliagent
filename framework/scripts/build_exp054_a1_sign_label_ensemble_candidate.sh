...
# Exp-054：A1 SIGN标签传播特征五折集成 + A2线上最佳候选包
#
# 依据：
# - Exp-053 最强配置 `sign_rw_k5_block_labelrw_h3_rownorm_w1`
#   无泄漏5 split均值 0.757078，显著高于 Exp-051 的线上 A1=0.7117。
# - 最强配置每个 split 的最佳后处理均为 model_only，因此提交推理默认
#   不再额外做 C&S 平滑（smooth_weight=0）。
# - A2 继续沿用当前线上最佳 Exp-044。
...
SRC_DIR="${SRC_DIR:-output/exp053_a1_sign_label_feature_audit/sign_rw_k5_block_labelrw_h3_rownorm_w1}"
OUT_DIR="${OUT_DIR:-output/exp054_submit_a1_sign_label_ensemble_a2_exp044}"
...
for ckpt in "${CHECKPOINTS[@]}"; do
  if [[ ! -f "$ckpt" ]]; then
    echo "缺少SIGN标签特征checkpoint: $ckpt"
    echo "请先运行：CUDA_VISIBLE_DEVICES=0 ./scripts/run_exp053_a1_sign_label_feature_audit.sh"
    exit 1
  fi
done
...
python3 code/a1_sign_infer.py \
  ...
  --output_json "$OUT_DIR/a1_sign_label_ensemble_infer.json"
